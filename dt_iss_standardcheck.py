...
for filename in os.listdir(path):
	if filename.startswith("s") or filename.startswith("k"):
		...
		for i in range(0,len(w)):
		     if w[i] > (1082.5+ref_value*5) and w[i]< (1082.9+ref_value*5): ### change back to the old wavelength range
		         w_temp.append(w[i])
		         p_temp.append(p[i])
		...
		os.mkdir("standard_"+fits_image_filename)
		os.chdir(r"/home/dtyler/Desktop/DocumentsDT/outputs/standard_"+fits_image_filename)
		...
		################################################################# v and (max_outlier>10)        and (max_outlier < len(x)-10))
			while ((abs(mdl(x[max_outlier]) - y[max_outlier]) > tolerance) and (icorr < 2)):
				icorr+=1
		...
		os.chdir(r"/home/dtyler/Desktop/DocumentsDT/Analysis/standard_eq_widths")
