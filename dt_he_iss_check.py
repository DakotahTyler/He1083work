...
for filename in os.listdir(path):
	if filename.startswith("k"):
		...
		name = fits_image_filename.split('.')[0]
		fits_image_filename = name
		...
		plt.legend(loc=2)
		plt.title(fits_image_filename.split('.')[0])
		plt.xlabel("Wavelength nm")
		plt.ylabel("Normalized Intensity")
		os.chdir(r"/home/dtyler/Desktop/DocumentsDT/outputs")
		os.mkdir(fits_image_filename)
		os.chdir(r"/home/dtyler/Desktop/DocumentsDT/outputs"/fits_image_filename)
		...
		tolerance = err_scaled*0.5 ### !!! reduced tolerance
		...
		if (len(outliers)>0):
			...
			while ((abs(mdl(x[max_outlier]) - y[max_outlier]) > tolerance) and (max_outlier>10) and (max_outlier < len(x)-10)) and (icorr < 4):
				...
				mdl = mdl + G2 ### !!! replaced G1 + G2 with this
				...
				plt.xlabel("Wavelength nm")
				plt.ylabel("Normalized Intensity")
				os.chdir(r"/home/dtyler/Desktop/DocumentsDT/outputs"/observation)
				plt.savefig(fits_image_filename[:-4]+"_model"+str(icorr+1)+".png")
				os.chdir(r"/home/dtyler/Desktop/DocumentsDT/Programs")
				plt.close()
